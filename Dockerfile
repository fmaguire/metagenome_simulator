...
LABEL version="1"
LABEL software="metagneome_simulator"
LABEL tags="Genomics"
...
# install metagenome_simulator
RUN git clone https://github.com/fmaguire/metagenome_simulator && cd metagenome_simulator && \
    cd build && cmake -Dtest=ON .. && make

# test metagenome_simulator
# RUN ./bin/run_unit_tests

# move to bin
RUN cp ./metagenome_simulator/build/bin/metagenome_simulator /usr/bin

# move to workdir 
WORKDIR /data/

# set rgi executable as cmd to allow overriding
# ENTRYPOINT ["metagenome_simulator"]
